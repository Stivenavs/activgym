...
$('#resendConsent').on('click', function() {
    ...
    var data = facturasTable.row(this).data();
    if(data && data.id) {
      window.location.href = "https://app.activgym.com.co/pdf/?id=" + encodeURIComponent(data.id) + "&type=invoice";
    } else {
  ...
  if ( ! $.fn.DataTable.isDataTable('#creditos-table') ) {
    ...
    // Al hacer clic en una fila de créditos, abrir el modal de edición
    $('#creditos-table tbody').on('click', 'tr', function() {
    ...
    // Actualizar el valor restante en el formulario de edición de crédito
    ...
    // Envío del formulario de edición de crédito con confirmación
...
<?php
if (strtotime($cliente['vencimiento_plan']) <= strtotime($hoy)) {
    // Si el plan está vencido (Para Vencidos)
?>
	<script>
// JS PARA PLANES VENCIDOS (MODIFICADO)
$(document).ready(function(){
  ...
  $("#first_payment_value, #second_payment_value").on("input", function () {
    limitarPagosDivididos();
  });
  ...
  $("#split_payment_check").change(function(){
    ...
    $("#secondPaymentWrapper").toggle(isChecked);
    $("#firstPaymentValueWrapper").toggle(isChecked);
    if (!isChecked) {
  ...
  $("#confirmPaymentBtn").off('click').on('click', function(){
    ...
    if (!paymentMethod) {
      alert("Seleccione un medio de pago"); processingPayment = false; return;
    }
    if (paymentMethod === 'Transferencia' && !bank) {
      alert("Seleccione un banco para la Transferencia"); processingPayment = false; return;
    }
    if (splitPayment && (!secondMethod || secondValue <= 0)) {
      alert("Complete la información del segundo medio de pago correctamente"); processingPayment = false; return;
    }
    ...
    Swal.fire({
      ...
      if (confirmResult.isConfirmed) {
        ...
        let pago_plan = `${yyyy}-${mm}-${dd}`;

        let planFrecuencia = <?php echo $planInfo ? intval($planInfo['frecuencia']) : 0; ?>;
        let planDias = <?php echo $planInfo ? intval($planInfo['dias']) : 0; ?>;
        let vencimientoDate = new Date(today);
        vencimientoDate.setMonth(vencimientoDate.getMonth() + planFrecuencia);
        vencimientoDate.setDate(vencimientoDate.getDate() + planDias - 1);
        let vencimiento_plan = `${vencimientoDate.getFullYear()}-${('0'+(vencimientoDate.getMonth()+1)).slice(-2)}-${('0'+vencimientoDate.getDate()).slice(-2)}`;
    ...
    // Si el plan no está vencido (Para No Vencidos)
?>
	<script>
// JS PARA PLANES NO VENCIDOS (MODIFICADO CON VALIDACIÓN EN TIEMPO REAL DE PAGOS DIVIDIDOS)
$(document).ready(function(){
  var originalTotal = <?php echo isset($planInfo['precio']) ? floatval($planInfo['precio']) : 0; ?>;
  var paymentModal = new bootstrap.Modal(document.getElementById('paymentModal'), { keyboard: false });
  var processingPayment = false;
  ...
  $("#confirmPaymentBtn").off('click').on('click', function(){
    ...
    Swal.fire({
      ...
      if (confirmResult.isConfirmed) {
        var pago_plan = "<?php echo $planInfo['pago_plan']; ?>";
        var vencimiento_plan = "<?php echo $planInfo['vencimiento_plan']; ?>";
